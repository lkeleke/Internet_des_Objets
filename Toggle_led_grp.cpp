...
namespace {
#define GROUP_NAME            "Groupe 7"
#define MQTT_TOPIC_PUBLISH      "seelec/"GROUP_NAME"/uplink"
#define MQTT_TOPIC_SUBSCRIBE    "seelec/"GROUP_NAME"/downlink"
#define SYNC_INTERVAL           1
#define MQTT_CLIENT_ID          "6LoWPAN_Node_"GROUP_NAME
#define PEER_GROUP_NAME         "Groupe 2"
#define MQTT_TOPIC_PEER_DOWNLINK "seelec/" PEER_GROUP_NAME "/downlink"
}
...
/*!
 *  \brief Publish to peer downlink: toggle ON/OFF
 */
static int8_t publish_peer_toggle()
{
    static bool peer_led_on = false;  
    peer_led_on = !peer_led_on;   
    MQTT::Message message;
    message.qos = MQTT::QOS1;
    message.retained = false;
    message.dup = false;
    message.payload = (void*)(peer_led_on ? "ON" : "OFF");
    message.payloadlen = strlen(peer_led_on ? "ON" : "OFF");

    printf("Sending: %s to peer group downlink\n", peer_led_on ? "ON" : "OFF");
    rc = client->publish(MQTT_TOPIC_PEER_DOWNLINK, message);
    if (rc != 0) {
        printf("Failed to publish: %d\n", rc);
        return rc;
    }
...
int main()
{
    ...
    // Publish
    button.fall(main_queue.event(publish_peer_toggle));
